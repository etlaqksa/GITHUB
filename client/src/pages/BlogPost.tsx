      ...
      <div className="max-w-4xl mx-auto" ref={swipeRef}>
        ...
        <div className="mt-10">
          ...
          <ArticleEndCTA
            articleTitle={title}
            serviceHint={
              (article.category || '').includes('حقن') || (article.categoryEn || '').toLowerCase().includes('grout')
                ? 'grouting'
                : (article.category || '').includes('كشف') || (article.categoryEn || '').toLowerCase().includes('cavity')
                  ? 'cavity'
                  : (article.category || '').includes('جيوفيزياء') || (article.categoryEn || '').toLowerCase().includes('geoph')
                    ? 'geophysical'
                    : 'general'
            }
          />
